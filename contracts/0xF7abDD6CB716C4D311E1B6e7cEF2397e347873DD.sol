...
function sub_1a94b350(?) payable {
    require block.number > stor9
    openPeriod.length = blocksBetween + stor9
    stor9 = blocksBetween + stor9 + (8 * 3600)
    stor6 = 0
    emit Closed(msg.sender);
}

function sub_305e89be(?) payable {
    if block.number <= openPeriod.length:
        revert with 0, 'openBifrost: not ready to open'
...
function enter(uint256 arg1) payable {
    ...
    require ext_code.size(stormAddress)
    call stormAddress.0x23b872dd with:
         gas gas_remaining wei
