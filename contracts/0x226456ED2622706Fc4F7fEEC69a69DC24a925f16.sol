...
function sub_f722bec1(?) payable {
    ...
    return address(sha3(0, this.address, arg1 + arg2, sha3(code.data[3488 len 4098])))
}
...
function sub_2351f51b(?) payable {
    ...
    if not uint32(ext_code.size(address(sha3(0, this.address, msg.sender + arg4, sha3(code.data[3488 len 4098]))))):
        create2 contract with 0 wei
                        salt: (msg.sender + arg4)
                        code: code.data[3488 len 4098]
        mem[4601 len 192] = 310925036, address(arg1) << 64, 0, address(arg2), address(arg3), key, 1, 0
        mem[4821 len 4] = 0
        call address(create2.new_address).0x1 with:
             gas gas_remaining wei
            args 0, mem[4793 len 4]
        if not return_data.size:
            call address(create2.new_address).0xccd8cbaf with:
                 gas gas_remaining wei
...
function run(address arg1, uint256 arg2) payable {
    ...
    while idx > 0:
        ...
        while s >= 32:
            ...
            s = s - 32
            continue 
        mem[_23 + floor32(mem[_24]) + 100] = mem[_24 + -(mem[_24] % 32) + floor32(mem[_24]) + 64 len mem[_24] % 32] or Mask(8 * -(mem[_24] % 32) + 32, -(8 * -(mem[_24] % 32) + 32) + 256, mem[_23 + floor32(mem[_24]) + 100])
        call arg1.mem[_23 + 100 len 4] with:
             gas gas_remaining wei
            args mem[_23 + 104 len _27 - 4]
        if return_data.size:
            mem[64] = _23 + ceil32(return_data.size) + 101
            mem[_23 + 100] = return_data.size
            mem[_23 + 132 len return_data.size] = ext_call.return_data[0 len return_data.size]
        idx = idx - 1
        continue 
}
