...
function transferOwnership(address arg1) {
    ...
    address(owner) = arg1
    require ext_code.size(dividendTrackerAddress)
    call dividendTrackerAddress.0x6d91196f with:
         gas gas_remaining wei
        args arg1
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
...
function skim() {
    ...
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    mem[ceil32(return_data.size) + 196 len 96] = unknown_0xa9059cbb(?????), Mask(224, 0, stor5), uint32(stor5), ext_call.return_data[0], mem[ceil32(return_data.size) + 196 len 28]
    call USDTAddress.mem[ceil32(return_data.size) + 196 len 4] with:
         gas gas_remaining wei
        args mem[ceil32(return_data.size) + 200 len 64]
    if not ext_call.success:
        revert with 0, 'call error'
}
...
function excludeMultipleAccountsFromFees(address[] arg1, bool arg2) {
    ...
    while idx < arg1.length:
        ...
        mem[32] = 28
        stor28[mem[(32 * idx) + 140 len 20]] = uint8(arg2)
        if idx == -1:
            revert with 0, 17
        idx = idx + 1
        continue 
