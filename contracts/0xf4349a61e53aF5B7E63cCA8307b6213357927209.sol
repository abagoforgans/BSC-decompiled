...
function ping(uint256 arg1) payable {
    mem[64] = 96
    ...
    while idx < arg1:
        ...
        mem[_12 + 132] = block.timestamp + 1
        require ext_code.size(swapAddress)
        call swapAddress.mem[mem[64] len 4] with:
             gas gas_remaining wei
            args mem[mem[64] + 4 len _12 + (32 * path.length) + -mem[64] + 192]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        _21 = mem[64]
...
function sell(uint256 arg1) {
    ...
    call path[path.length].field_0.0x70a08231 with:
         gas gas_remaining wei
        args this.address
