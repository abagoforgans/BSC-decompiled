...
function withdraw() {
    ...
    if userReferralBonus[address(msg.sender)].field_3328 <= 0:
        revert with 0, 'User has no dividends'
    userReferralBonus[address(msg.sender)].field_256 = block.timestamp
    if eth.balance(this.address) >= userReferralBonus[address(msg.sender)].field_3328:
        call msg.sender with:
           value userReferralBonus[address(msg.sender)].field_3328 wei
             gas 2300 * is_zero(value) wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        emit Withdrawn(userReferralBonus[address(msg.sender)].field_3328, msg.sender);
    else:
        call msg.sender with:
