...
function mint(address arg1) payable {
    ...
    if stor12.length <= 0:
        ...
        if not arg1:
            revert with 0, 'ERC721: mint to the zero address'
        if ownerOf[stor12[stor12.length]]:
            revert with 0, 'ERC721: token already minted'
        stor9[stor12[stor12.length]] = tokenByIndex.length
        tokenByIndex.length++
        tokenByIndex[tokenByIndex.length] = stor12[stor12.length]
        if arg1:
            if not arg1:
                revert with 0, 'ERC721: balance query for the zero address'
            tokenOfOwnerByIndex[address(arg1)][stor3[address(arg1)]] = stor12[stor12.length]
            stor7[stor12[stor12.length]] = balanceOf[address(arg1)]
        else:
            if tokenByIndex.length < 1:
                revert with 'NH{q', 17
            if tokenByIndex.length - 1 >= tokenByIndex.length:
                revert with 'NH{q', 50
            if stor9[stor12[stor12.length]] >= tokenByIndex.length:
                revert with 'NH{q', 50
            tokenByIndex[stor9[stor12[stor12.length]]] = tokenByIndex[tokenByIndex.length]
            stor9[stor8[stor8.length]] = stor9[stor12[stor12.length]]
            stor9[stor12[stor12.length]] = 0
            if not tokenByIndex.length:
                revert with 'NH{q', 49
            tokenByIndex[tokenByIndex.length] = 0
            tokenByIndex.length--
        if balanceOf[address(arg1)] > -2:
            revert with 'NH{q', 17
        balanceOf[address(arg1)]++
        ownerOf[stor12[stor12.length]] = arg1
        emit Transfer(0, arg1, stor12[stor12.length]);
        if ext_code.size(arg1) > 0:
            require ext_code.size(arg1)
            call arg1.onERC721Received(address arg1, address arg2, uint256 arg3, bytes arg4) with:
                 gas gas_remaining wei
                args 0, uint32(msg.sender), 0, stor12[stor12.length], 128, 0
            if not ext_call.success:
                if not return_data.size:
                    revert with 0, 'ERC721: transfer to non ERC721Receiver implementer'
                if not return_data.size:
