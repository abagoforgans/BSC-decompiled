...
const INIT_CODE_PAIR_HASH = sha3(code.data[2645 len 11616], mem[23424 len 8], Mask(128, 64, code.data[14269 len 16]) >> 64)
...
function setFeeTo(address arg1) payable {
    require calldata.size - 4 >= 32
    if not arg1:
        revert with 0, 'SeaDEXSwap: ZERO ADDRESS'
    if feeToSetterAddress != msg.sender:
        revert with 0, 'SeaDEXSwap: FORBIDDEN'
    feeToAddress = arg1
}

function setFeeToSetter(address arg1) payable {
    require calldata.size - 4 >= 32
    if not feeToSetterAddress:
        revert with 0, 'SeaDEXSwap: ZERO ADDRESS'
    if feeToSetterAddress != msg.sender:
        revert with 0, 'SeaDEXSwap: FORBIDDEN'
...
function createPair(address arg1, address arg2) payable {
    require calldata.size - 4 >= 64
    if arg1 == arg2:
        revert with 0, 'SeaDEXSwap: IDENTICAL_ADDRESSES'
    if arg1 < arg2:
        if not arg1:
            revert with 0, 'SeaDEXSwap: ZERO_ADDRESS'
        if pair[address(arg1)][address(arg2)]:
            revert with 0, 'SeaDEXSwap: PAIR_EXISTS'
        create2 contract with 0 wei
                        salt: sha3(arg1, arg2)
                        code: code.data[2645 len 11640]
