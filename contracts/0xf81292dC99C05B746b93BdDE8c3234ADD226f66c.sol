...
const getAdd = address(sha3(0, address(this.address), tx.origin << 96, sha3(code.data[3658 len 3987])))
...
function add() payable {
    if not uint32(ext_code.size(address(sha3(0, this.address, msg.sender, sha3(code.data[3658 len 3987]))))):
        create2 contract with 0 wei
                        salt: msg.sender
                        code: code.data[3658 len 3987]
        call address(create2.new_address).0x1 with:
             gas gas_remaining wei
            args Mask(1504, -1280, 310925036, 0x73feaa1ee314f8c655e35423, 2479481422, 0xe09fabb73bd3ade0a17ecc321fd13a19e81ce82, 0x9cf7bc57584b7998236eff51b98a168dcea9b0, key, 1, 0) << 1280, mem[4665 len 4]
}
