...
function run(bytes arg1) payable {
    ...
    if address(cd[(arg1 + 164)]):
        call address(cd[(arg1 + 36)]).swap(uint256 rg1, uint256 rg2, address rg3, bytes rg4) with:
             gas gas_remaining wei
            args 0, 997 * cd[(arg1 + 100)] * Mask(112, 0, ext_call.return_data[32]) / (997 * cd[(arg1 + 100)]) + (1000 * Mask(112, 0, ext_call.return_data[0])), address(cd[(arg1 + 164)]), 128, 0, None
    else:
        call address(cd[(arg1 + 36)]).swap(uint256 rg1, uint256 rg2, address rg3, bytes rg4) with:
             gas gas_remaining wei
            args 0, 997 * cd[(arg1 + 100)] * Mask(112, 0, ext_call.return_data[32]) / (997 * cd[(arg1 + 100)]) + (1000 * Mask(112, 0, ext_call.return_data[0])), address(this.address), 128, 0, None
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
}
