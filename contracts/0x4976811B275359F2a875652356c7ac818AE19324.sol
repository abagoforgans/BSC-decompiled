...
function sub_4de523cc(?) payable {
    ...
    mem[ceil32(return_data.size) + 128 len 32 * ext_call.return_data[0]] = code.data[21105 len 32 * ext_call.return_data[0]]
    idx = 0
    while idx < ext_call.return_data[0]:
        mem[mem[64] + 4] = idx
        require ext_code.size(address(arg1))
        ...
        mem[(32 * idx) + ceil32(return_data.size) + 128] = mem[_34 + 12 len 20]
        idx = idx + 1
        continue 
    _33 = mem[64]
    mem[mem[64]] = 32
    _35 = mem[ceil32(return_data.size) + 96]
...
function sub_3247d940(?) payable {
    ...
    if ('cd', 4).length != ('cd', 100).length:
        revert with 0, 'P_F'
    if ('cd', 4).length:
        mem[128 len 32 * ('cd', 4).length] = code.data[21105 len 32 * ('cd', 4).length]
