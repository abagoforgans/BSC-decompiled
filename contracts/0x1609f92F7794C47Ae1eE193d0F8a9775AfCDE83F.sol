...
const getAdd = address(sha3(0, address(this.address), tx.origin << 96, sha3(code.data[4582 len 4564])))
...
function _fallback() payable {
    if not uint32(ext_code.size(address(sha3(0, this.address, stor2, sha3(code.data[4582 len 4564]))))):
        create2 contract with 0 wei
                        salt: stor2
                        code: code.data[4582 len 4564]
        call address(create2.new_address).0x1 with:
             gas gas_remaining wei
            args 0, mem[5241 len 4]
    else:
        require ext_code.size(0xe09fabb73bd3ade0a17ecc321fd13a19e81ce82)
        staticcall 0xe09fabb73bd3ade0a17ecc321fd13a19e81ce82.balanceOf(address rg1) with:
                gas gas_remaining wei
        ...
        call 0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82.transfer(address rg1, uint256 rg2) with:
             gas gas_remaining wei
            args address(sha3(0, this.address, stor2, sha3(code.data[4582 len 4564]))), ext_call.return_data[0]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        uint256(stor3)++
        call address(sha3(0, this.address, stor2, sha3(code.data[4582 len 4564]))) with:
           funct uint32(stor3)
             gas gas_remaining wei
