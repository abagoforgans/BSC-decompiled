...
const INIT_CODE_PAIR_HASH = sha3(code.data[3964 len 15968], mem[32128 len 8], Mask(128, 64, code.data[19940 len 16]) >> 64)


uint8 stor0;
address feeToAddress; offset 8
...
function setFeeTo(address arg1) payable {
    ...
    if feeToSetterAddress != msg.sender:
        revert with 0, 'StableX: FORBIDDEN'
    feeToAddress = arg1
}
...
function createPair(address arg1, address arg2) payable {
    require calldata.size - 4 >= 64
    if arg1 == arg2:
        revert with 0, 'StableX: IDENTICAL_ADDRESSES'
    if arg1 < arg2:
        if not arg1:
            revert with 0, 'StableX: ZERO_ADDRESS'
        if pair[address(arg1)][address(arg2)]:
            revert with 0, 'StableX: PAIR_EXISTS'
        create2 contract with 0 wei
                        salt: sha3(arg1, arg2)
                        code: code.data[3964 len 15992]
