...
function sub_3cad8b22(?) {
    ...
    idx = 0
    while idx < _70:
        _91 = mem[(32 * ('cd', 4).length) + 128]
        s = 0
        while s < _91:
            require idx < mem[96]
            _93 = mem[(32 * idx) + 128]
            require s < mem[(32 * ('cd', 4).length) + 128]
            _95 = mem[(32 * s) + (32 * ('cd', 4).length) + 160]
            _96 = mem[64]
            _97 = mem[mem[(32 * s) + (32 * ('cd', 4).length) + 160]]
            idx = 0
            while idx < _97:
                mem[_96 + idx] = mem[_95 + idx + 32]
                _70 = mem[96]
                _91 = mem[(32 * ('cd', 4).length) + 128]
                idx = idx + 32
                continue 
            mem[_96 + _97] = 0
            mem[mem[64] + 4] = address(stor[sha3(mem[mem[64] len _96 + _97 + -mem[64] + 32])])
            mem[mem[64] + 36] = -1
            require ext_code.size(address(_93))
            call address(_93).approve(address rg1, uint256 rg2) with:
