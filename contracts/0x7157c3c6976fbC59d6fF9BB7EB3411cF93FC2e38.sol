...
function getTotalShares() {
    if stor24 < 1000000000 * 10^18:
        revert with 0, 17
    return (stor24 - 1000000000 * 10^18)
}

function getBurntTokens() {
    if 1000000000 * 10^18 < _circulatingSupply:
        revert with 0, 17
    return (-_circulatingSupply + 1000000000 * 10^18)
}
...
function UpdateLimits(uint256 arg1, uint256 arg2) {
    ...
    if owner != msg.sender:
        if marketingWalletAddress != msg.sender:
            if stor13 != msg.sender:
                revert with 0, 'Caller not authorized'
    if arg1 < 142857142857142857142857142:
