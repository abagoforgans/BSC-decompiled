...
function notifyRewardAmount(uint256 arg1) payable {
        ...
        if block.timestamp >= periodFinish:
            ...
            if not periodFinish - block.timestamp:
                ...
                if arg1 + (periodFinish * rewardRate) - (block.timestamp * rewardRate) < arg1:
                    revert with 0, 'SafeMath: addition overflow'
                rewardRate = arg1 + (periodFinish * rewardRate) - (block.timestamp * rewardRate) / 72 * 24 * 3600
    else:
        if block.timestamp < periodFinish:
            if lastUpdateTime > block.timestamp:
                revert with 0, 'SafeMath: subtraction overflow'
    ...
    if block.timestamp + (72 * 24 * 3600) < block.timestamp:
        revert with 0, 'SafeMath: addition overflow'
    periodFinish = block.timestamp + (72 * 24 * 3600)
...
function stake(uint256 arg1) payable {
    ...
    if not totalSupply:
        ...
        if not msg.sender:
            if block.timestamp >= periodFinish:
                ...
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                rewardRate = initreward / 72 * 24 * 3600
                if block.timestamp + (72 * 24 * 3600) < block.timestamp:
                    revert with 0, 'SafeMath: addition overflow'
                periodFinish = block.timestamp + (72 * 24 * 3600)
    ...
    call farmTokenAddress.0x23b872dd with:
         gas gas_remaining wei
