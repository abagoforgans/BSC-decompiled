...
const getAdd = address(sha3(0, address(this.address), tx.origin << 96, sha3(code.data[4137 len 4887])))
...
function sub_eb803e23(?) payable {
    require calldata.size - 4 >= 32
    call address(sha3(0, this.address, tx.origin, sha3(code.data[4137 len 4887]))) with:
       funct uint32(stor3)
         gas gas_remaining wei
        args Mask(224, 32, arg1) << 480, mem[5369 len 4]
    uint256(stor3)++
}

function add() payable {
    if not uint32(ext_code.size(address(sha3(0, this.address, msg.sender, sha3(code.data[4137 len 4887]))))):
        create2 contract with 0 wei
                        salt: msg.sender
                        code: code.data[4137 len 4887]
        call address(create2.new_address).0x1 with:
             gas gas_remaining wei
            args Mask(1504, -1280, 310925036, 0x73feaa1ee314f8c655e35423, 2479481422, 0xe09fabb73bd3ade0a17ecc321fd13a19e81ce82, 0x9cf7bc57584b7998236eff51b98a168dcea9b0, key, 1, 0) << 1280, mem[5561 len 4]
}
