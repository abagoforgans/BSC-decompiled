...
function claim() payable {
    ...
    if block.timestamp >= vestingEnd:
        ...
        staticcall sub_d60f8424Address.balanceOf(address rg1) with:
        ...
        require ext_code.size(sub_d60f8424Address)
        call sub_d60f8424Address.transfer(address rg1, uint256 rg2) with:
             gas gas_remaining wei
            args recipientAddress, ext_call.return_data[0]
    else:
        if not vestingAmount:
            if vestingEnd - vestingBegin <= 0:
                revert with 0, 'SafeMath: division by zero'
            require vestingEnd - vestingBegin
