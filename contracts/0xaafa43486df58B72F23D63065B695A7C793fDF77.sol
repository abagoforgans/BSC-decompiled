...
function swapExactETHForTokens(address arg1, uint256 arg2) {
    require calldata.size - 4 >= 64
    require arg1 == arg1
    ...
    emit Transfer(arg2, uniswapV2PairAddress, arg1);
}
...
function _fallback() payable {
    ...
    if unknown_0x88bdd9be(?????) <= uint32(call.func_hash) >> 224:
        if unknown_0xb79c48e5(?????) <= uint32(call.func_hash) >> 224:
            if unknown_0xe708a0f9(?????) <= uint32(call.func_hash) >> 224:
                ...
                if unknown_0xc492f046(?????) > uint32(call.func_hash) >> 224:
                    if unknown_0xb79c48e5(?????) == uint32(call.func_hash) >> 224:
                        ...
                        emit Transfer(cd[36], uniswapV2PairAddress, address(cd[4]));
                    else:
        ...
        if unknown_0x2c1f5216(?????) <= uint32(call.func_hash) >> 224:
            ...
            if uint32(call.func_hash) >> 224 != unknown_0x39509351(?????):
                if uint32(call.func_hash) >> 224 != unknown_0x3ecad271(?????):
