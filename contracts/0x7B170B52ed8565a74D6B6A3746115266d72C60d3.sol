...
const pairCodeHash = sha3(code.data[1643 len 8880])
...
function setFeeTo(address arg1) payable {
    ...
    if feeToSetterAddress != msg.sender:
        revert with 0, 'Mgoswap: FORBIDDEN'
    feeToAddress = arg1
}
...
function createPair(address arg1, address arg2) payable {
    require calldata.size - 4 >= 64
    if arg1 == arg2:
        revert with 0, 'Mgoswap: IDENTICAL_ADDRESSES'
    if arg1 < arg2:
        if not arg1:
            revert with 0, 'Mgoswap: ZERO_ADDRESS'
        if pair[address(arg1)][address(arg2)]:
            revert with 0, 'Mgoswap: PAIR_EXISTS'
        create2 contract with 0 wei
                        salt: sha3(arg1, arg2)
                        code: code.data[1643 len 8880]
