...
function setFeeTo(address arg1) payable {
    ...
    if feeToSetterAddress != msg.sender:
        revert with 0, 'Marblecake: FORBIDDEN'
    feeToAddress = arg1
}
...
function createPair(address arg1, address arg2) payable {
    require calldata.size - 4 >= 64
    if arg1 == arg2:
        revert with 0, 'Marblecake: IDENTICAL_ADDRESSES'
    if arg1 < arg2:
        if not arg1:
            revert with 0, 'Marblecake: ZERO_ADDRESS'
        if pair[address(arg1)][address(arg2)]:
            revert with 0, 'Marblecake: PAIR_EXISTS'
