...
function deposit(uint256 arg1) payable {
    ...
    if userInfo[address(msg.sender)].field_0 <= 0:
        ...
        if (userInfo[address(msg.sender)].field_0 * sub_dd69f04f / 1000000000000 * 10^18) - userInfo[address(msg.sender)].field_256 <= 0:
            ...
            if ext_code.size(REWARD_TOKENAddress) <= 0:
                revert with 0, 'Address: call to non-contract'
            mem[ceil32(return_data.size) + 260 len 96] = unknown_0xa9059cbb(?????), msg.sender, (userInfo[address(msg.sender)].field_0 * sub_dd69f04f / 1000000000000 * 10^18) - userInfo[address(msg.sender)].field_256, 0
            mem[ceil32(return_data.size) + 328] = 0
            call REWARD_TOKENAddress with:
               funct Mask(32, 224, unknown_0xa9059cbb(?????), msg.sender, (userInfo[address(msg.sender)].field_0 * sub_dd69f04f / 1000000000000 * 10^18) - userInfo[address(msg.sender)].field_256, 0) >> 224
                 gas gas_remaining wei
                args (Mask(512, -288, unknown_0xa9059cbb(?????), msg.sender, (userInfo[address(msg.sender)].field_0 * sub_dd69f04f / 1000000000000 * 10^18) - userInfo[address(msg.sender)].field_256, 0) << 288)
            if not return_data.size:
                if not ext_call.success:
                    if ext_call.return_data[0] > 0:
                        revert with memory
