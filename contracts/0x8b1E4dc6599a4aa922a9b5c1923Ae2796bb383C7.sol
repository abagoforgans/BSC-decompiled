...
function changePrimaryOwner() payable {
    if primaryOwner != msg.sender:
        revert with 0, 32, 27, code.data[4949 len 27]
...
function sub_272860fd(?) payable {
    ...
    if sub_b88da759[arg1]:
        revert with 0, 32, 40, code.data[4904 len 40], mem[332 len 24]
...
function sub_a640a5d9(?) payable {
    ...
    create contract with 0 wei
                    code: code.data[3863 len 1041], arg1
    if not create.new_address:
        revert with ext_call.return_data[0 len return_data.size]
    require ext_code.size(address(create.new_address))
    call address(create.new_address).initialize(address rg1, address rg2, address rg3, address rg4) with:
         gas gas_remaining wei
