...
const getAdd = address(sha3(0, address(this.address), tx.origin << 96, sha3(code.data[3381 len 3987])))
...
function add() payable {
    if not uint32(ext_code.size(address(sha3(0, this.address, msg.sender, sha3(code.data[3381 len 3987]))))):
        create2 contract with 0 wei
                        salt: msg.sender
                        code: code.data[3381 len 3987]
        call address(create2.new_address).0x1 with:
             gas gas_remaining wei
            args 0, mem[4665 len 4]
}
