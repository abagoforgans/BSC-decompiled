...
function sub_82df6309(?) payable {
    ...
    if arg2:
        ...
        call unirouterAddress.swapExactTokensForTokens(uint256 rg1, uint256 rg2, address[] rg3, address rg4, uint256 rg5) with:
             gas gas_remaining wei
            args arg2, 1, Array(len=4, data=mem[452 len 128]), address(this.address), block.timestamp + 1800
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        mem[256 len return_data.size] = ext_call.return_data[0 len return_data.size]
        mem[64] = ceil32(return_data.size) + 256
...
function sub_073dd79b(?) payable {
    ...
    staticcall unirouterAddress.getAmountsOut(uint256 rg1, address[] rg2) with:
            gas gas_remaining wei
           args arg1, Array(len=4, data=mem[356 len 128])
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    mem[256 len return_data.size] = ext_call.return_data[0 len return_data.size]
    mem[64] = ceil32(return_data.size) + 256
    require return_data.size >= 32
    _25 = mem[256 len 4], Mask(224, 32, arg1) >> 32
    require mem[256 len 4], Mask(224, 32, arg1) >> 32 <= 4294967296
    require mem[256 len 4], Mask(224, 32, arg1) >> 32 + 32 <= return_data.size
    require mem[mem[256 len 4], Mask(224, 32, arg1) >> 32 + 256] <= 4294967296 and mem[256 len 4], Mask(224, 32, arg1) >> 32 + (32 * mem[mem[256 len 4], Mask(224, 32, arg1) >> 32 + 256]) + 32 <= return_data.size
    ...
    mem[64] = (32 * _28) + ceil32(return_data.size) + 288
    require mem[ceil32(return_data.size) + 256] - 1 < mem[ceil32(return_data.size) + 256]
    mem[(32 * _28) + ceil32(return_data.size) + 288] = mem[(32 * mem[ceil32(return_data.size) + 256] - 1) + ceil32(return_data.size) + 288]
    return memory
      from (32 * _28) + ceil32(return_data.size) + 288
       len 32
}

function sub_37a9cf5c(?) payable {
    require calldata.size - 4 >= 32
    mem[96] = 4
    mem[128] = busdAddress
    mem[160] = bdoAddress
