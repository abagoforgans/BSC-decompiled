...
function emergencyWithdraw(uint256 arg1) payable {
    ...
    if owner != msg.sender:
        revert with 0, 'wOwnable: caller is not the owne'
    if not ext_code.size(sub_7cb68945Address):
...
function withdraw() payable {
    ...
    if not totalReward:
        ...
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        totalReward = ext_call.return_data[0]
        require ext_code.size(address(chefAddress))
        call address(chefAddress).deposit(uint256 rg1, uint256 rg2) with:
             gas gas_remaining wei
            args poolId, 0
    ...
    if not sub_fb81f917[msg.sender]:
        ...
        if 0 / poolAmount > totalReward:
            revert with 0, 'SafeMath: subtraction overflow'
        totalReward -= 0 / poolAmount
        if sub_fb81f917[msg.sender] > poolAmount:
            revert with 0, 'SafeMath: subtraction overflow'
        ...
        if return_data.size:
            ...
            if not mem[356]:
                revert with 0, 
                            ...
                            0x725361666545524332303a204552433230206f7065726174696f6e20646964206e6f7420737563636565,
                            mem[ceil32(return_data.size) + 435 len 22]
        if totalReward * sub_fb81f917[msg.sender] / poolAmount > totalReward:
            revert with 0, 'SafeMath: subtraction overflow'
