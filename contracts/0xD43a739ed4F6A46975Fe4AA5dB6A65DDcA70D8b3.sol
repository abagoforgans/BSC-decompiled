...
function migrate() payable {
                ...
                if not periodFinish - lastUpdateTime:
                    ...
                    if not (periodFinish * rewardRate) - (lastUpdateTime * rewardRate):
                        ...
                        if not balanceOf[address(msg.sender)]:
                            ...
                            rewards[address(msg.sender)] += ((10^18 * periodFinish * rewardRate) - (10^18 * lastUpdateTime * rewardRate) / totalSupply * balanceOf[address(msg.sender)]) + (rewardPerTokenStored * balanceOf[address(msg.sender)]) - (userRewardPerTokenPaid[address(msg.sender)] * balanceOf[address(msg.sender)]) / 10^18
        userRewardPerTokenPaid[address(msg.sender)] = rewardPerTokenStored
    if not migrateToAddress:
        revert with 0, 'no migration destination'
    require ext_code.size(migrateToAddress)
    staticcall migrateToAddress.factory() with:
            gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_code.size(stakingTokenAddress)
    staticcall stakingTokenAddress.factory() with:
            gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    if ext_call.return_data[12 len 20] == address(ext_call.return_data[0]):
        revert with 0, 'cannot migrate'
    ...
    if ext_call.return_data[0] <= 0:
        revert with 0, 'has no tokens to migrate'
    ...
    call stakingTokenAddress.approve(address rg1, uint256 rg2) with:
         gas gas_remaining wei
        args this.address, balanceOf[msg.sender]
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_code.size(stakingTokenAddress)
    call stakingTokenAddress.0x23b872dd with:
         gas gas_remaining wei
        args this.address, stakingTokenAddress, balanceOf[msg.sender]
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_code.size(stakingTokenAddress)
    call stakingTokenAddress.burn(address rg1) with:
         gas gas_remaining wei
        args migrateToAddress
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 64
    require ext_code.size(migrateToAddress)
    call migrateToAddress.mint(address rg1) with:
         gas gas_remaining wei
        args msg.sender
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
