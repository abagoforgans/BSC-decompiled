...
function deposit(uint256 arg1) payable {
    ...
    if not return_data.size:
        ...
        if not arg1:
            ...
            if not ext_code.size(wantAddress):
                revert with 0, 'Address: call to non-contract'
            mem[520 len 64] = 0, address(farmContractAddress), Mask(224, 31, ext_call.return_data[0]) >> 31
            call wantAddress with:
               funct uint32(stor3)
                 gas gas_remaining wei
        ...
        if not return_data.size:
            ...
            if not wantLockedTotal:
                ...
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                return arg1
        ('bool', ('stor', ('name', 'wantLockedTotal', 10)))
        if not arg1:
            if not wantLockedTotal:
                revert with 0, 'SafeMath: division by zero'
            sharesTotal += 0 / wantLockedTotal
            ...
            if not ext_code.size(wantAddress):
                revert with 0, 'Address: call to non-contract'
            mem[ceil32(return_data.size) + 521 len 64] = 0, address(farmContractAddress), Mask(224, 31, ext_call.return_data[0]) >> 31
