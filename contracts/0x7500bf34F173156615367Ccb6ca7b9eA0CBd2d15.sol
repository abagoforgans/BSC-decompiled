...
const onERC1155BatchReceived(address arg1, address arg2, uint256[] arg3, uint256[] arg4, bytes arg5) = 0xbc197c8100000000000000000000000000000000000000000000000000000000

const onERC1155Received(address arg1, address arg2, uint256 arg3, uint256 arg4, bytes arg5) = 0xf23a6e6100000000000000000000000000000000000000000000000000000000
...
function handleBridgedTokens(address arg1, uint256 arg2, bytes arg3, bytes32 arg4) {
    ...
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require ext_code.size(mediatorContractOnOtherSide['erc1155token'])
    call mediatorContractOnOtherSide['erc1155token'].safeTransferFrom(address rg1, address rg2, uint256 rg3, uint256 rg4, bytes rg5) with:
         gas gas_remaining wei
        args address(this.address), address(arg1), arg2, 1, Array(len=arg3.length, data=arg3[all])
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
}
    ...
    if not arg1:
        ...
        if not ext_call.success:
            create contract with ('balance', 'address') wei
                            code: code.data[11873 len 33], arg2
...
function initialize(address arg1, address arg2, address arg3, uint256 arg4, address arg5) {
    if stor4[Mask(104, 0, 'isInitialized', 0)]:
        revert with 0, 'BasicMediator1155: already initialized.'
    if ext_code.size(arg1) <= 0:
        revert with 0, 'AMBMediator: _bridgeContract is not contract.'
    mediatorContractOnOtherSide[Mask(112, 0, 'bridgeContract', 0)] = arg1
    mediatorContractOnOtherSide['mediatorContract'] = arg2
    if ext_code.size(arg3) <= 0:
        revert with 0, 'ERC1155Bridge: toekn is not contract.'
    ...
    mem[530] = sha3(Mask(160, 96, uint64(this.address), 0, 0) >> 96)
    mem[594] = 'nonce'
    mem[562] = 5
    mem[599 len 0] = None
    mem[604 len 27] = Mask(176, 0, 'nonce'), mem[626 len 5]
    mem[599 len 5] = 'nonce', 0 % 1099511627776
    stor3['nonce', 0 % 1099511627776][] = Array(len=uint64(this.address), 0, 0, data=mem[530 len uint64(this.address), 0, 0])
