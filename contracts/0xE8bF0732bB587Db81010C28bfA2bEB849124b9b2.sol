...
function sub_a67b23a1(?) {
    require not isPaused
    require stor4[address(msg.sender)]
    if arg2:
        require ext_code.size(bLinkTokenAddr)
        call bLinkTokenAddr.0x70a08231 with:
             gas gas_remaining wei
            args this.address
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        ...
        require ext_code.size(bLinkTokenAddr)
        call bLinkTokenAddr.0xa9059cbb with:
             gas gas_remaining wei
...
function updateVersion(address[] arg1) {
    ...
    if not arg1.length:
        ...
        mem[(32 * arg1.length) + 160 len 32 * arg1.length] = code.data[7324 len 32 * arg1.length]
        require 0 < arg1.length
        mem[(32 * arg1.length) + 160] = eth.balance(this.address)
        s = 0
        s = 0
