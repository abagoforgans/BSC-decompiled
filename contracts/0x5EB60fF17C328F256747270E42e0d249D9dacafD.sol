...
function setLiquidityProviderAddress(address arg1) {
    require calldata.size - 4 >= 32
    require arg1 == arg1
    if owner == msg.sender:
...
function getCirculatingSupply() {
    if totalSupply < balanceOf[stor1]:
        revert with 'NH{q', 17
...
function sub_85dd4ce1(?) {
    ...
    while idx < ('cd', 4).length:
        ...
        mem[32] = 23
        stor23[address(cd[((32 * idx) + cd[4] + 36)])] = 1
        if idx == -1:
            revert with 'NH{q', 17
        idx = idx + 1
        continue 
