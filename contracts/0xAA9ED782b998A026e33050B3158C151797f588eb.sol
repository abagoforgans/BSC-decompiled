...
function withdraw2(address arg1, uint256 arg2) payable {
    ...
    if arg2:
        ...
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        mem[228 len 96] = unknown_0x23b872dd(?????), address(this.address) << 64, 0, msg.sender, ext_call.return_data[0 len 28]
...
function sub_187a8fc3(?) payable {
    ...
    if arg1 < arg2:
        ...
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        mem[196 len 64] = unknown_0xa9059cbb(?????), address(arg3) << 64, 0, Mask(224, 32, arg5) >> 32
        call arg1 with:
             gas gas_remaining wei
            args Mask(224, 32, arg5) << 224, mem[260 len 4]
        if not return_data.size:
            if not ext_call.success:
        ...
        require ext_code.size(arg3)
        if arg1 != arg1:
            call arg3.swap(uint256 rg1, uint256 rg2, address rg3) with:
                 gas gas_remaining wei
                args ext_call.return_data[0], 0, this.address
        else:
            call arg3.swap(uint256 rg1, uint256 rg2, address rg3) with:
                 gas gas_remaining wei
                args 0, ext_call.return_data[0], this.address
    else:
        if arg2 == arg1:
            staticcall arg4.getAmountOut(uint256 rg1, uint256 rg2, uint256 rg3) with:
                    gas gas_remaining wei
