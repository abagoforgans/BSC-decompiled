...
function balanceOfBatch(address[] arg1, uint256[] arg2) {
    ...
    if arg1.length:
        mem[128 len 32 * arg1.length] = code.data[9255 len 32 * arg1.length]
    idx = 0
...
function getFeeBps(uint256 arg1) {
    ...
    mem[_28 + 32 len 32 * _27] = code.data[9255 len 32 * _27]
    _52 = mem[96]
    idx = 0
    while idx < _52:
        require idx < mem[96]
...
function mint(uint256 arg1, address arg2) {
    ...
    mem[388] = uint256(stor0.field_0)
    idx = 388
    s = 0
    while stor0.length + 388 > idx + 32:
    ...
    call this.address.0xf242432a with:
         gas gas_remaining wei
        args this.address, address(arg2), arg1, 1, Array(len=stor0.length, data=mem[388 len stor0.length + (floor32(stor0.length - 1) + -stor0.length + 32 % 32)])
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
}
...
function uri(uint256 arg1) {
    ...
    if tokenURIPrefix.length + stor4[arg1].length:
        mem[ceil32(stor4[arg1].length) + ceil32(tokenURIPrefix.length) + 192 len 32 * tokenURIPrefix.length + stor4[arg1].length] = code.data[9255 len 32 * tokenURIPrefix.length + stor4[arg1].length]
    idx = 0
    s = 0
