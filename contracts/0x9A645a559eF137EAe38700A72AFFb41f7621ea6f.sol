...
function claim() {
    ...
    if ext_call.return_data[0] + totalClaimed <= sub_f0809455:
        ...
        if not ext_code.hash(tokenAddress):
            revert with 0, 'SafeERC20: call to non-contract'
        mem[ceil32(return_data.size) + 361 len 64] = unknown_0xa9059cbb(?????), msg.sender, Mask(224, 0, offeredOf[address(msg.sender)].field_32)
        mem[ceil32(return_data.size) + 453 len 4] = uint32(offeredOf[address(msg.sender)].field_64)
        call tokenAddress with:
           funct uint32(msg.sender)
             gas gas_remaining wei
            args Mask(480, 0, offeredOf[address(msg.sender)].field_0), mem[ceil32(return_data.size) + 425 len 4]
        if not return_data.size:
            require not ext_call.success
            revert with 0, 'SafeERC20: low-level call failed'
        ...
        emit Claim(address arg1, uint256 arg2, uint256 arg3):
                   offeredOf[address(msg.sender)].field_0,
                   totalClaimed,
