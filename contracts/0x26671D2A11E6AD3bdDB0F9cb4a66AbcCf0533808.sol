...
function rewardsAvailable() payable {
    require ext_code.size(address(chefAddress))
    staticcall address(chefAddress).pendingReward(address arg1, uint256 arg2) with:
            gas gas_remaining wei
           args this.address, poolId
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    return ext_call.return_data[0]
...
function deposit() payable {
    ...
    if ext_call.return_data[0]:
        ...
        call address(chefAddress).0xe2bbb158 with:
             gas gas_remaining wei
            args ext_call.return_data[0], poolId
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require ext_code.size(address(chefAddress))
        staticcall address(chefAddress).0x93f1a40b with:
...
function withdraw(uint256 arg1) payable {
    ...
    if ext_call.return_data[0] < arg1:
        ...
        call address(chefAddress).0x441a3e70 with:
             gas gas_remaining wei
            args arg1 - ext_call.return_data[0], poolId
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require ext_code.size(wantAddress)
        staticcall wantAddress.0x70a08231 with:
