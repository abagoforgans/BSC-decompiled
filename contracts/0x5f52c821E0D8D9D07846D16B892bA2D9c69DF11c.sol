...
function sub_3b20673e(?) payable {
    require calldata.size - 4 >= 6400
    mem[96 len 3200] = call.data[4 len 3200]
    mem[3296 len 3200] = call.data[3204 len 3200]
    mem[6496] = 0
    if owner != msg.sender:
        revert with 0, 'Ownable: caller is not the owner'
    idx = 0
    while uint8(idx) < 100:
        if mem[(32 * uint8(idx)) + 108 len 20]:
            require uint8(idx) < 100
            if mem[(32 * uint8(idx)) + 3296]:
                require uint8(idx) < 100
                call mem[(32 * uint8(idx)) + 108 len 20] with:
                   value mem[(32 * uint8(idx)) + 3296] wei
                     gas 2300 * is_zero(value) wei
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                require uint8(idx) < 100
                _23 = mem[(32 * uint8(idx)) + 96]
                mem[6496] = mem[(32 * uint8(idx)) + 3296]
                emit Transfer(mem[6496], msg.sender, address(_23));
        idx = idx + 1
        continue 
    return 1
}
