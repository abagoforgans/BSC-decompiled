...
function testSell() {
    ...
    call 0x10ed43c718714eb63d5aa57b78b54704e256024e.swapExactTokensForTokens(uint256 arg1, uint256 arg2, address[] arg3, address arg4, uint256 arg5) with:
         gas gas_remaining wei
        args 50 * 10^18, 0, 160, address(this.address), block.timestamp + 15, 2, mem[388 len 64]
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    mem[192 len return_data.size] = ext_call.return_data[0 len return_data.size]
    mem[64] = ceil32(return_data.size) + 192
    require return_data.size >= 32
    require mem[192 len 4], 11641532182 <= test266151307()
    require mem[192 len 4], 11641532182 + 223 < return_data.size + 192
    if mem[mem[192 len 4], 11641532182 + 192] > test266151307():
        revert with 0, 65
    if ceil32(return_data.size) + floor32(mem[mem[192 len 4], 11641532182 + 192]) + 193 > test266151307() or floor32(mem[mem[192 len 4], 11641532182 + 192]) + 1 < 0:
        revert with 0, 65
    require return_data.size >= mem[192 len 4], 11641532182 + (32 * mem[mem[192 len 4], 11641532182 + 192]) + 32
}
