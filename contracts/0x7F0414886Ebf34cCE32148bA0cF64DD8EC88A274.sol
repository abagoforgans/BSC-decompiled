...
uint256 sub_0f8aa2d3;
uint256 sub_4d0ebc09;
...
function sub_4c738a5e(?) payable {
    return sub_4c738a5eAddress
}

function sub_4d0ebc09(?) payable {
    return sub_4d0ebc09
}
...
function sub_5efbb7fd(?) payable {
    ...
    if ext_call.return_data[31 len 1] == 1:
        revert with 0, 32, 18, 0xe0b98d52616e6765206e6f74206d617463680000000000000000000000000000
...
function sub_4fd2eb57(?) payable {
    ...
    if arg2 == 2:
        ...
        if arg2 != 3:
            ...
            if sub_074f7ea2[address(arg1)][arg2 << 248].field_256 <= 0:
                return stor17
    ('gt', ('field', 256, ('stor', ('map', ('mask_shl', 8, 0, 248, ('param', 'arg2')), ('map', ('mask_shl', 160, 0, 0, ('param', 'arg1')), ('name', 'sub_074f7ea2', 25))))), 0)
    return sub_074f7ea2[address(arg1)][arg2 << 248].field_256
}
...
function startReward(uint256 arg1) payable {
    require calldata.size - 4 >= 32
    if address(_governanceAddress) != msg.sender:
        revert with 0, 'not governance'
    if sub_5e7ec827:
        if block.timestamp < _periodFinish:
            ...
            if not block.timestamp - _lastUpdateTime:
                ...
                if _rewardPerTokenStored + (0 / sub_5e7ec827 / 1000) < _rewardPerTokenStored:
                    revert with 0, 'SafeMath: addition overflow'
                _rewardPerTokenStored += 0 / sub_5e7ec827 / 1000
            else:
                require block.timestamp - _lastUpdateTime
                if (block.timestamp * _rewardRate) - (_lastUpdateTime * _rewardRate) / block.timestamp - _lastUpdateTime != _rewardRate:
                    revert with 0, 32, 33, 0xfe536166654d6174683a206d756c7469706c69636174696f6e206f766572666c6f, mem[325 len 31]
                if not (block.timestamp * _rewardRate) - (_lastUpdateTime * _rewardRate):
                    ...
                    if _rewardPerTokenStored + ((10^18 * block.timestamp * _rewardRate) - (10^18 * _lastUpdateTime * _rewardRate) / sub_5e7ec827 / 1000) < _rewardPerTokenStored:
                        revert with 0, 'SafeMath: addition overflow'
                    _rewardPerTokenStored += (10^18 * block.timestamp * _rewardRate) - (10^18 * _lastUpdateTime * _rewardRate) / sub_5e7ec827 / 1000
        else:
            if _lastUpdateTime > _periodFinish:
                revert with 0, 'SafeMath: subtraction overflow'
            if not _periodFinish - _lastUpdateTime:
                ...
                if not (_periodFinish * _rewardRate) - (_lastUpdateTime * _rewardRate):
                    ...
                    if _rewardPerTokenStored + ((10^18 * _periodFinish * _rewardRate) - (10^18 * _lastUpdateTime * _rewardRate) / sub_5e7ec827 / 1000) < _rewardPerTokenStored:
                        revert with 0, 'SafeMath: addition overflow'
                    _rewardPerTokenStored += (10^18 * _periodFinish * _rewardRate) - (10^18 * _lastUpdateTime * _rewardRate) / sub_5e7ec827 / 1000
    if block.timestamp < _periodFinish:
...
function notifyRewardAmount(uint256 arg1) payable {
    ...
    if not sub_5e7ec827:
        if block.timestamp < _periodFinish:
            ...
            _lastUpdateTime = _periodFinish
        if ext_code.size(sub_86c2ef68Address) <= 0:
            revert with 0, 'Address: call to non-contract'
        mem[292 len 96] = unknown_0x23b872dd(?????), msg.sender, address(this.address), Mask(224, 32, arg1) >> 32
        mem[416 len 4] = 0
        mem[388 len 0] = 0
        call sub_86c2ef68Address with:
             gas gas_remaining wei
        ...
        if not return_data.size:
            ...
            if block.timestamp >= _periodFinish:
                ...
                if not _periodFinish - block.timestamp:
                    ...
                    require _periodFinish - block.timestamp
                    if (_periodFinish * _rewardRate) - (block.timestamp * _rewardRate) / _periodFinish - block.timestamp != _rewardRate:
                        revert with 0, 32, 33, 0xfe536166654d6174683a206d756c7469706c69636174696f6e206f766572666c6f, mem[457 len 31]
                    if arg1 + (_periodFinish * _rewardRate) - (block.timestamp * _rewardRate) < arg1:
                        revert with 0, 'SafeMath: addition overflow'
                    _rewardRate = arg1 + (_periodFinish * _rewardRate) - (block.timestamp * _rewardRate) / 168 * 24 * 3600
        else:
