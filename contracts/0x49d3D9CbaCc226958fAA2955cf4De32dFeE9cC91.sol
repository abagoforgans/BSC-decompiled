...
function withdrawTo(address arg1) payable {
    ...
    staticcall address(stor2.field_0).balanceOf(address rg1) with:
            gas gas_remaining wei
           args this.address
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    ...
    require ext_call.return_data[0] == ext_call.return_data[0]
    require ext_code.size(address(stor2.field_0))
    call address(stor2.field_0).transfer(address rg1, uint256 rg2) with:
         gas gas_remaining wei
...
function test2() payable {
    ...
    require return_data.size >= 32
    require ext_call.return_data[0] == ext_call.return_data[0]
    mem[(4 * ceil32(return_data.size)) + 480] = 64
    if not uint8(stor2.field_160):
        mem[(4 * ceil32(return_data.size)) + 352] = uint8((sha3(block.hash(block.number - 1), ext_call.return_data[0]) % ext_call.return_data[0]) + 1)
    else:
        mem[(4 * ceil32(return_data.size)) + 352] = uint8((sha3(block.hash(block.number - 1), ext_call.return_data[0] + 1) % ext_call.return_data[0]) + 1)
    mem[(4 * ceil32(return_data.size)) + 384] = uint8((sha3(block.hash(block.number - 1), ext_call.return_data[0] + 10 * 10^18) % ext_call.return_data[0]) + 1)
    mem[(4 * ceil32(return_data.size)) + 416] = uint8((sha3(block.hash(block.number - 1), block.timestamp) % ext_call.return_data[0]) + 1)
    mem[(4 * ceil32(return_data.size)) + 448] = uint8((sha3(block.hash(block.number - 1), block.difficulty) % ext_call.return_data[0]) + 1)
    idx = 0
    s = (4 * ceil32(return_data.size)) + 352
