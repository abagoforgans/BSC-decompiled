            ...
            if ext_call.return_data[0] > earnLowerlimit:
                ...
                if not ext_call.return_data[0]:
                    ...
                    call tokenAddress with:
                       funct uint32(stor9)
                         gas gas_remaining wei
                        args Mask(480, -256, unknown_0xa9059cbb(?????), Mask(224, 0, stor9), uint32(stor9), 0) << 256, mem[520 len 4]
                    if not return_data.size:
                        if not ext_call.success:
                            revert with 0, 'SafeERC20: low-level call failed'
                        if not unknown_0x23b872dd(?????), Mask(224, 32, msg.sender) >> 32:
                ...
                if ext_call.return_data[0] > earnLowerlimit:
                    ...
                    if not ext_call.return_data[0]:
                        ...
                        call tokenAddress with:
                           funct uint32(stor9)
                             gas gas_remaining wei
                            args Mask(480, -256, unknown_0xa9059cbb(?????), Mask(224, 0, stor9), uint32(stor9), 0) << 256, mem[ceil32(return_data.size) + 521 len 4]
                        if not return_data.size:
                            if not ext_call.success:
                                revert with 0, 'SafeERC20: low-level call failed'
                            if not unknown_0x23b872dd(?????), Mask(224, 32, msg.sender) >> 32:
