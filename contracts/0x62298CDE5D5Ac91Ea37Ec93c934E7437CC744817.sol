    ...
    if not arg3:
        ...
        if poolInfo[arg1].field_256 != arg2:
            ...
            while idx < poolInfo.length:
                ...
                s = poolInfo[idx].field_256 + s
                continue 
            if -stor[(4 * stor7.length) + _429 + 1] + (poolInfo.length * stor[(4 * stor7.length) + _429 + 1]):
                require 0 < poolInfo.length
                if uint256(poolInfo.field_256) > totalAllocPoint:
                    revert with 0, 'SafeMath: subtraction overflow'
                if -stor[(4 * stor7.length) + _429 + 1] + (poolInfo.length * stor[(4 * stor7.length) + _429 + 1]) / 3 < 0:
                    revert with 0, 'SafeMath: addition overflow'
                totalAllocPoint = (-stor[(4 * stor7.length) + _429 + 1] + (poolInfo.length * stor[(4 * stor7.length) + _429 + 1]) / 3) + totalAllocPoint - uint256(poolInfo.field_256)
                require 0 < poolInfo.length
                uint256(poolInfo.field_256) = -stor[(4 * stor7.length) + _429 + 1] + (poolInfo.length * stor[(4 * stor7.length) + _429 + 1]) / 3
}
