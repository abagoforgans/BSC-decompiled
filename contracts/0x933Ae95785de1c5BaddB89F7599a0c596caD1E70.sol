...
function notifyRewardAmount(uint256 arg1) payable {
        ...
        if block.timestamp >= periodFinish:
            ...
            if not periodFinish - block.timestamp:
                ...
                if arg1 + (periodFinish * rewardRate) - (block.timestamp * rewardRate) < arg1:
                    revert with 0, 'SafeMath: addition overflow'
                rewardRate = arg1 + (periodFinish * rewardRate) - (block.timestamp * rewardRate) / 2400 * 24 * 3600
    else:
        if block.timestamp < periodFinish:
            if lastUpdateTime > block.timestamp:
                revert with 0, 'SafeMath: subtraction overflow'
    ...
    if block.timestamp + (2400 * 24 * 3600) < block.timestamp:
        revert with 0, 'SafeMath: addition overflow'
    periodFinish = block.timestamp + (2400 * 24 * 3600)
...
function stake(uint256 arg1) payable {
    ...
    if not totalSupply:
        ...
        if not msg.sender:
            ...
            if block.timestamp <= 445417 * 3600:
                revert with 0, 'Cannot stake yet'
