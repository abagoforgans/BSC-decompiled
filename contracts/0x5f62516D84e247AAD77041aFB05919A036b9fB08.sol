...
const getAdd = address(sha3(0, address(this.address), tx.origin << 96, sha3(code.data[3001 len 3987])))
...
function add() payable {
    if not uint32(ext_code.size(address(sha3(0, this.address, msg.sender, sha3(code.data[3001 len 3987]))))):
        create2 contract with 0 wei
                        salt: msg.sender
                        code: code.data[3001 len 3987]
