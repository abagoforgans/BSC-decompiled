...
const initCodeHash = sha3(code.data[1915 len 8896], mem[17984 len 15])

const INIT_CODE_HASH = sha3(code.data[1915 len 8896], mem[17984 len 15])
...
function setFeeTo(address arg1) payable {
    ...
    if feeToSetterAddress != msg.sender:
        revert with 0, 'Triangle: FORBIDDEN'
    feeToAddress = arg1
}
...
function createPair(address arg1, address arg2) payable {
    require calldata.size - 4 >= 64
    if arg1 == arg2:
        revert with 0, 'Triangle: IDENTICAL_ADDRESSES'
    if arg1 < arg2:
        if not arg1:
            revert with 0, 'Triangle: ZERO_ADDRESS'
        if pair[address(arg1)][address(arg2)]:
            revert with 0, 'Triangle: PAIR_EXISTS'
        create2 contract with 0 wei
                        salt: sha3(arg1, arg2)
                        code: code.data[1915 len 8911]
